...
$getUser = doMysql("SELECT * FROM users WHERE username='".safe($_GET['wow'])."' OR oldusrname='".safe($_GET['wow'])."'");
$getWows = doMysql("SELECT * FROM wow WHERE wow_to='".safe($getUser['r']['id'])."'");
...
if($_GET['mask'] != 1) # Only draw the username when the mask isn't applied
	drawText("a", 16, $getUser['r']['username'], alloColor($getUser['r']['usrname_color'], true)); # %Username%
...
function alloColor($r, $g = NULL, $b = NULL, $alpha = 0, $image = NULL){
	...
	if($image === NULL)
		$image = $im;
	if(strlen($r) > 3){ # The function is given a hexcode
